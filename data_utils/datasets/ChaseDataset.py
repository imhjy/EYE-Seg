...
class ChaseDataset(Dataset):
    ...
    def __init__(self, hypes, train: bool, transforms=None, fold=0):
        ...
        if fold == 0:
            ...
            self.label = [os.path.join(data_root, "label", i.split(".")[0] + "_1stHO.png")
                           for i in img_names]
            # check files
            for i in self.label:
    ...
    def fold_dataset_split(hypes):
        ...
        label_list = np.array(
            [os.path.join('label', file.split(".")[0] + "_1stHO.png") for file in os.listdir(os.path.join(dataset_path, 'images'))]
        )
