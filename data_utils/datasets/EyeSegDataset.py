...
class EyeSegDataset(Dataset):
    ...
    def __init__(self, hypes, train: bool, transforms=None, fold=0):
        super(EyeSegDataset, self).__init__()
        ...
        if fold == 0:
            ...
            self.label = [os.path.join(data_root, "label", i.split(".")[0] + ".png")
                           for i in img_names]
            # check files
            for i in self.label:
            ...
            self.mask = [os.path.join(data_root, "mask", i.split(".")[0] + ".png") for i in img_names]
            # check files
            for i in self.mask:
                if os.path.exists(i) is False:
                    raise FileNotFoundError(f"file {i} does not exists.")
    ...
    def fold_dataset_split(hypes):
        ...
        mask_list = np.array(
            [os.path.join('mask', file.split(".")[0] + ".png") for file in os.listdir(os.path.join(dataset_path, 'images'))]
        )
        # manual_list = np.array(os.listdir(os.path.join(train_path, '1st_manual')) + os.listdir(
        #     os.path.join(test_path, '1st_manual')))

        label_list = np.array(
            [os.path.join('label', file.split(".")[0] + ".png") for file in os.listdir(os.path.join(dataset_path, 'images'))]
        )
